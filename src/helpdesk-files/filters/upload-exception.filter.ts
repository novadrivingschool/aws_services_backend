/* src/helpdesk-files/filters/upload-exception.filter.ts
 *
 * Turns multer's terse errors into messages that tell the user what is allowed.
 *
 * `@nestjs/platform-express` already maps `LIMIT_FILE_SIZE` to a 413 and the
 * other multer limits to a 400, but the body it produces is just
 * `{"message":"File too large"}` — which does not say how large is too large.
 * This filter rewrites those specific responses and leaves everything else
 * untouched.
 */
import {
  ArgumentsHost,
...
import { POLICY_SUMMARY } from '../helpdesk-files.constants';

/** Verbatim messages multer produces, mapped to something actionable. */
...
export class UploadExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(UploadExceptionFilter.name);
    ...
    if (!override) {
      // Not a multer limit error — preserve the original response exactly.
