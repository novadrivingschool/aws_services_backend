/* src/loa-files/loa-files.service.ts
 *
 * Leave of Absence attachment storage. Owns its own S3 client on purpose —
 * mirrors helpdesk-files.service.ts. This module must not inherit anything
 * from S3Service, whose methods accept a client-supplied folder and filename.
 *
...
} from './loa-files.constants';
import { declaredTypeMatches, detectMimeType } from './utils/file-signature.util';
import { buildObjectKey, extensionOf, isKeyOwnedByLoa } from './utils/loa-key.util';

export interface UploadedLoaAttachment {
  success: true;
  key: string;
...
export interface SignedLoaAttachmentUrl {
  success: true;
  url: string;
  expiresIn: number;
}
...
@Injectable()
export class LoaFilesService {
  private readonly logger = new Logger(LoaFilesService.name);
  ...
  async upload(
    ...
    const key = buildObjectKey(loaId, detected);
    ...
    this.logger.log(
      `Stored ${key} (${detected}, ${mb(file.size)} MB) for LOA ${loaId} by ${actor}`,
    );
  ...
  async getSignedUrl(key: string, loaId: string): Promise<SignedLoaAttachmentUrl> {
    this.assertOwnership(key, loaId);
    ...
    try {
      ...
      if (status === 404 || status === 403) {
        this.logger.warn(`Attachment referenced by LOA ${loaId} is not in the bucket: ${key}`);
  ...
  async remove(key: string, loaId: string, actor = 'unknown'): Promise<{ success: true }> {
    this.assertOwnership(key, loaId);
  ...
  /** The single choke point that keeps one LOA record from reaching another's objects. */
  private assertOwnership(key: string, loaId: string): void {
    if (!isKeyOwnedByLoa(key, loaId)) {
      this.logger.warn(`Blocked out-of-scope key "${key}" for LOA ${loaId}`);
